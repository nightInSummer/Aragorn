...
(function() { 
     ...
     * @return {Number}
     */
    GeoUtils.getMaximum = function(polyline, type1, type2) {
        //检查类型
        if(polyline instanceof BMap.Polyline || 
            polyline instanceof Array){
            //将polyline统一为数组
            ...
            if (type1 === 'min' && type2 === 'lng') {
                var minLongitude = pts[0].lng;  
                for (var i = 0; i < pts.length; i++) {
                    if (pts[i].lng < minLongitude) {
                      minLongitude = pts[i].lng;  
                    }
                }  
                return minLongitude; 
            }
            if (type1 === 'max' && type2 === 'lng') {
                var maxLongitude = pts[0].lng;  
                for (var i = 0; i < pts.length; i++) {
                    if (pts[i].lng > maxLongitude) {
                      maxLongitude = pts[i].lng;  
                    }
                }  
                return maxLongitude; 
            }
            if (type1 === 'min' && type2 === 'lat') {
                var minLatitude  = pts[0].lat;  
                for (var i = 0; i < pts.length; i++) {
                    if (pts[i].lat < minLatitude) {
                      minLatitude = pts[i].lat;  
                    }
                }  
                return minLatitude; 
            }
            if (type1 === 'max' && type2 === 'lat') {
                var maxLatitude = pts[0].lat;
                for (var i = 0; i < pts.length; i++) {
                    if (pts[i].lat > maxLatitude) {
                      maxLatitude = pts[i].lat;  
                    }
                }  
                return maxLatitude; 
            }
     ...
     * @return {Point}
     */
    GeoUtils.getPolylineMidpoint = function(polyline) {
        //检查类型
        if(polyline instanceof BMap.Polyline || 
            polyline instanceof Array){
            //将polyline统一为数组
            ...
            let lat = (GeoUtils.getMaximum(pts, 'min', 'lat') + GeoUtils.getMaximum(pts, 'max', 'lat')) / 2;
            let lng = (GeoUtils.getMaximum(pts, 'min', 'lng') + GeoUtils.getMaximum(pts, 'max', 'lng')) / 2;
     ...
     * @returns {Point}
     */
    GeoUtils.getCenterOfGravityPoint = function(polygon){
       //检查类型
        ...
        for (var i = 1; i <= pts.length; i++) {
           var iLat = pts[i % pts.length].lat;  
           var iLng = pts[i % pts.length].lng;  
           var nextLat = pts[i - 1].lat;  
           var nextLng = pts[i - 1].lng;  
