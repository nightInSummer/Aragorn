    ...
    return <div className="excalibur-table">
      ...
      <table cellSpacing="0">
        <tbody>
          ...
          {state.get('data').map((item: Map<string, any>) => <tr>{state.get('columns').map((col : Map<string, any>) => 
            item.get(col.get('key')) ? <td title={(col.get('text').toJS ? col.get('text').toJS() : col.get('text')) + ' :'}>{item.get(col.get('key')).toJS ? item.get(col.get('key')).toJS() : item.get(col.get('key'))}</td> : <td style={{height: 0, padding: 0}}/>
          ).toJS()}</tr>).toJS()}
        </tbody>
      </table>
    </div>
